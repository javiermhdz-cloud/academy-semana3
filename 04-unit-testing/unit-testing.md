...
2) **Dónde se ve en tu código:**
   ...
   - Mockito:
      - Creacion e inyeccion de dobles Mockito: `PrimerMockTest.java` tiene la anotacion `@Mock` para inyectar objetos (dobles automatizados) de `RepositorioAlumnos`.
      - Programacion de respuestas: `RespuestasTest.java` usa `when(...).thenReturn(...)` para las respuestas.
      - Verificacion de invocacion de metodos: `VerificarTest.java` implementa `verify()`.
      - Inyeccion de Dependencias Reales: `ServicioInscripcionTest.java` utiliza la anotacion previamente discuta `@InjectMocks` para la inyeccion de dependencias reales en el codigo.
